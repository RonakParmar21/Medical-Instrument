...
<html>
    ...
    <body>
        ...
        <% 
            int sub_cat_id = Integer.parseInt(request.getParameter("subId"));
                    try {
                Connection con = ConnectionProvider.getConnection();
                String query = "select * from sub_category where sub_cat_id='"+ sub_cat_id +"'";
            ...
            <form action="../UpdateSubCategoryDetailsServlet" method="POST" enctype="multipart/form-data">
                <h1 style="text-align: center">Add Main Category</h1> <hr><br>
            
                <label>Category Name</label>
                <input type="hidden" name="main_cat_id" value="<%= rs.getInt(2) %>"/>
                <input type="hidden" name="sub_cat_id" value="<%= rs.getInt(1) %>" />
                <input type="text" name="sub_cat_name" value="<%= rs.getString(3) %>"  placeholder="Enter Main Category Name" required />
                <label>Choose Image</label>
                <input type="file" name="sub_cat_image" />
